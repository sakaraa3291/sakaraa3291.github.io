...
function renderSchematicRoute(body,v,s,d,c,laps,routeOverride,compact){
 ...
 map.append(svg('path',{d:geo.loopPath,fill:'none',stroke:'#26374A','stroke-width':20,'stroke-linecap':'round'}));
 // 走行区間（ゴールから距離分だけ遡った範囲）を本線色で重ねる
 const runLen=Math.min(geo.d,geo.lap)*geo.k;
 map.append(svg('path',{d:geo.loopPath,fill:'none',stroke:'#31465B','stroke-width':20,'stroke-linecap':'butt','stroke-dasharray':`${runLen.toFixed(3)} ${(geo.total-runLen).toFixed(3)}`}));
 ...
 for(const a of route.anchors){
  ...
  let dy=pt.zone==='back'?-18:down?28:-18;const dx=isStart?0:isGoal?10:0;
 ...
 left.append(map);
 const bands=svg('svg',{viewBox:'0 0 580 96',class:'profileSvg',role:'img','aria-label':`${v}${s}${d}m 距離目盛`});
 const X=m=>28+(580-56)*m/d;
 bands.append(svg('line',{x1:X(0),x2:X(d),y1:48,y2:48,stroke:'#41566B','stroke-width':4}));
 let lastLabel=-99;
...
function renderAnchoredRoute(body,v,s,d,c,laps){
 ...
 left.append(map);
 const bands=svg('svg',{viewBox:'0 0 580 96',class:'profileSvg',role:'img','aria-label':`${v}${s}${d}m 公式情報に基づく起伏アンカー`});
