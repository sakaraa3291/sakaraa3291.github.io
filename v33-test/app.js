...
function renderTokyo1600Route(body,c,d,laps){
 const route=c.route,prof=bundle['elevation.json'].profiles['東京|芝|1600'];
 if(!route||route.mode!=='distance-anchored-schematic'||d!==1600||route.distance_m!==d||!prof)return false;
 ...
 const map=svg('svg',{viewBox:'0 0 580 270',class:'courseSvg',role:'img','aria-label':'東京芝1600m 距離アンカー付き模式コース図'});
 ...
 for(const a of route.anchors){
  ...
  map.append(svg('circle',{cx:pt.x.toFixed(2),cy:pt.y.toFixed(2),r:5,fill:'#0E161F',stroke:'#E9EFF5','stroke-width':2}),
 ...
 map.append(svg('text',{x:360,y:35,fill:'#8195A8','font-size':10,'text-anchor':'middle'},'向正面'),svg('text',{x:470,y:236,fill:'#8195A8','font-size':10,'text-anchor':'middle'},'ホームストレッチ'));
 left.append(map);
 const bands=svg('svg',{viewBox:'0 0 580 96',class:'profileSvg',role:'img','aria-label':'東京芝1600m 公式情報に基づく起伏アンカー'});
 const X=m=>28+(580-56)*m/geo.goal;
 bands.append(svg('line',{x1:X(0),x2:X(geo.goal),y1:48,y2:48,stroke:'#41566B','stroke-width':4}));
 let lastLabel=-99;
 ...
 if(hill)bands.append(svg('line',{x1:X(hill.from_m),x2:X(hill.to_m),y1:48,y2:48,stroke:'#FF6B4A','stroke-width':10,'stroke-linecap':'round'}),svg('text',{x:(X(hill.from_m)+X(hill.to_m))/2,y:30,fill:'#FF9A82','font-size':10,'text-anchor':'middle'},`上り坂 ${Math.round(hill.to_m-hill.from_m)}m / 高低差${hill.rise_m.toFixed(1)}m`));
 left.append(bands);
 const f1=node('div',undefined,'fact');f1.append(node('div','公式アンカー','fk'),node('div','スタート＝2C付近 ／ 3Cまで約540m（JRA公式の概数） ／ 直線525.9m','fv'));
 const f2=node('div',undefined,'fact');f2.append(node('div','直線の坂','fk'),node('div','残り460m地点〜残り300m地点。全長160m・高低差2m。坂後はゴールまで約300m','fv'));
 const f3=node('div',undefined,'fact');f3.append(node('div','精度','fk'),node('div','距離比例の模式図。形状は模式、距離目安のみ公式記載。実測座標ではありません。','fv'));
 right.append(f1,f2,f3);
 grid.append(left,right);body.append(grid);
 for(const q of prof.qualitative)body.append(node('p','起伏（位置範囲未確定）：'+q.label,'note'));
 body.append(node('p','図の形状は模式ですが、1mあたりの長さは全区間そろえています。ラップ色（200m区間・端数距離対応）と距離アンカーは同じ距離目盛の上にあります。坂の1140m・1300mはJRA公表の「直線、残り460m地点から300m地点」を1600mへ換算した値です。3コーナーまで約540mはJRA公式の週次レース解説に明記された概数です。コース区分別の細かな差は未整理です。','note'));
 if(c.source_url){const a=node('a','JRA 東京競馬場 コース紹介（直線・高低差・坂の残り距離）');a.href=c.source_url;a.target='_blank';a.rel='noopener noreferrer';body.append(a)}
 if(c.route_source_url){body.append(node('span',' ／ '));const a=node('a','JRA公式 週次コース解説（約540m）');a.href=c.route_source_url;a.target='_blank';a.rel='noopener noreferrer';body.append(a)}
 return true;
}
function renderCourse(v,s,d,laps){
 ...
 body.append(facts);
 if(v==='東京'&&s==='芝'&&d===1600&&renderTokyo1600Route(body,c,d,laps))return;
