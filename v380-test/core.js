...
(function(root){
'use strict';
const APP_VERSION='3.8.0', SCHEMA=2;
...
function validateBundle(b){
 ...
 // 実施場を訂正した記録は、訂正前後と出典が揃っていなければ受け付けない
 const gnotes=lap.meta.audit?.grade_notes;
 if(gnotes!==undefined){
  assert(Array.isArray(gnotes),'重賞注記不正');
  for(const n of gnotes)assert(obj(n)&&text(n.race)&&text(n.text)&&Array.isArray(n.sources)&&n.sources.length>0&&n.sources.every(text)&&lap.grade[n.race]!==undefined,'重賞注記不正');
 }
 const gone=lap.meta.audit?.grade_excluded;
 if(gone!==undefined){
  assert(Array.isArray(gone),'重賞除外記録不正');
  for(const x of gone)assert(obj(x)&&text(x.race)&&text(x.source)&&text(x.reason)&&Array.isArray(x.years)&&x.years.length>0&&lap.grade[x.race]===undefined,'重賞除外記録不正');
 }
 const fixes=lap.meta.audit?.grade_venue_fixes;
 if(fixes!==undefined){
  assert(Array.isArray(fixes),'収録場訂正記録不正');
  for(const f of fixes)assert(obj(f)&&text(f.race)&&text(f.from)&&text(f.to)&&f.from!==f.to&&text(f.source)&&text(f.note)&&lap.grade[f.race]?.v===f.to,'収録場訂正記録不正');
 }
 ...
 for(const [k,p] of Object.entries(elev.profiles)){
  ...
  if(p.shape!==undefined){
   const sh=p.shape;
   assert(obj(sh)&&Array.isArray(sh.segments)&&sh.segments.length>0&&Array.isArray(sh.quotes)&&sh.quotes.length>0&&sh.quotes.every(text)&&Array.isArray(sh.sources)&&sh.sources.length>0&&sh.sources.every(text),'起伏形状不正');
   assert(sh.total_m===null||num(sh.total_m,0,20),'起伏合計不正');
   let prev=-1;
   for(const sg of sh.segments){
    assert(obj(sg)&&num(sg.from_rem,0)&&num(sg.to_rem,0)&&sg.to_rem>sg.from_rem&&sg.from_rem>=prev-0.6,'起伏区間順序不正');
    assert(['up','down','flat','unknown'].includes(sg.dir),'起伏向き不正');
    assert(sg.rise_m===null||num(sg.rise_m,0,20),'起伏区間高低差不正');
    assert(['numeric','direction-only'].includes(sg.precision)&&text(sg.src),'起伏区間出典不正');
    // 数値が無い区間に numeric を名乗らせない
    assert(sg.precision!=='numeric'||num(sg.rise_m,0,20),'数値なし区間のnumeric表記');
    prev=sg.from_rem;
   }
  }
