...
(function(root){
'use strict';
const APP_VERSION='3.4.0', SCHEMA=2;
...
function validateBundle(b){
 ...
 for(const [k,c] of Object.entries(b['courses.json'].courses)){
  ...
  if(c.route!==undefined){
   ...
   for(const role of ['start','first_corner','straight_entry','goal'])assert(roles.has(role),'経路ロール不足');
   const at=role=>r.anchors.find(a=>a.role===role).m;
   assert(at('start')<at('first_corner')&&at('first_corner')<at('straight_entry')&&at('straight_entry')<at('goal'),'経路アンカー順序不正');
  }
 }
